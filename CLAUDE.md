...
- `pkg/server/` — MCP tool layer. `server.go` registers two tools: `web_search` (delegates to the Searxng client, results formatted by `formatSearchResults`) and `web_read`. `reader.go` does generic HTML→Markdown, while `reader_reddit.go` and `reader_github.go` special-case Reddit threads (via `.json`) and GitHub issues/PRs (via API, combining issue/PR body + comments). `fetchURLContent` dispatches to the right reader based on URL shape.
